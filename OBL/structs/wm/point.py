...
class Point(WMEntity):
    ...
    def __init__(self, point_ref, *args, **kwargs):
        ...
        self.coordinate_system = kwargs.get(
        ...
        if len(nodes) == 1:
            ...
            self.x, self.y = self._convert_to_cartesian(
                nodes[0].lat, nodes[0].lon)
        else:
            self.logger.error(
                "No node found with given ref {}".format(point_ref))
            raise Exception(
                "No node found with given ref {}".format(point_ref))
    ...
    def _assign_parent_attributes(self):
        __, __, relations = self.osm_adapter.get_parent(
            id=self.id, data_type='node', parent_child_role='topology')
        if len(relations) > 0:
