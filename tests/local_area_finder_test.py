...
class TestLocalAreaFinder(unittest.TestCase):
    ...
    def test_given_area_name_inside_la_with_xy(self):
        ...
        y = -20.23611060809344
        local_area = self.osm_bridge.get_local_area(x=x, y=y, area_name=area_name, isLatlong=False)
    ...
    def test_given_area_name_inside_la_with_latlong(self):
        ...
        latitude = 50.1361671
        longitude = 8.6476004
        local_area = self.osm_bridge.get_local_area(x=latitude, y=longitude, area_name=area_name, isLatlong=True)
        self.assertEqual(local_area.ref, expected_local_area_ref)
    ...
    def test_not_given_area_name(self):
        ...
        latitude = 50.1361677
        longitude = 8.6475883
        local_area = self.osm_bridge.get_local_area(x=latitude, y=longitude, isLatlong=True, floor_name="AMK_L-1")
        self.assertEqual(local_area.ref, expected_local_area_ref)
    ...
    def test_given_area_name_with_behaviour(self):
        ...
        local_area = self.osm_bridge.get_local_area(area_name=area_name, behaviour="docking")
        self.assertEqual(local_area.ref, expected_local_area_ref)

        area_name = "AMK_A_L-1_RoomBU21"
        expected_local_area_ref = "AMK_A_L-1_RoomBU21_LA1"
        local_area = self.osm_bridge.get_local_area(area_name=area_name, behaviour="undocking")
        self.assertEqual(local_area.ref, expected_local_area_ref)
    ...
    def test_not_given_area_name_with_behaviour(self):
        ...
        floor_name = "AMK_L-1"
        expected_local_area_ref = "AMK_D_L-1_C41_LA2"
        local_area = self.osm_bridge.get_local_area(floor_name=floor_name, behaviour="docking")
        self.assertEqual(local_area.ref, expected_local_area_ref)

        floor_name = "AMK_L-1"
        expected_local_area_ref = "AMK_A_L-1_RoomBU21_LA1"
        local_area = self.osm_bridge.get_local_area(floor_name=floor_name, behaviour="undocking")
        self.assertEqual(local_area.ref, expected_local_area_ref)
