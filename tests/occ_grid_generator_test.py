...
class TestOccGridGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.global_origin = [48.6938905, 9.1902848] # logimat (coordinates of node id 57)
    ...
    def test_require_server_info_and_global_origin(self):
        self.assertRaises(Exception, OccGridGenerator(
            server_ip=self.server_ip, server_port=self.server_port, debug=self.debug))
    ...
    def test_map_files_exists_with_bridge_object(self):
        occ_grid_generator = OccGridGenerator(
            ...
            dirname='~',
            debug=self.debug)
        filename = occ_grid_generator.generate_map(floor=0)
        print(filename)
